...
class DECODERLAYER(torch.nn.Module):
    ...
    def __init__(self,NUM_HEAD,EMBED_DIM,use_casual,MLP_ratio, *args, **kwargs):
        ...
        self.MHA_DECODER=MULTIHEADSELFATTN(NUM_HEAD=NUM_HEAD, EMBED_DIM=EMBED_DIM,use_casual=use_casual,attention_type=None)
        
        self.norm1 = torch.nn.LayerNorm(self.EMBED_DIM)
        
        self.MHA_ENCODER_DECODER=MULTIHEADSELFATTN(NUM_HEAD=NUM_HEAD,EMBED_DIM=EMBED_DIM,use_casual=False,attention_type=None)
    ...
    def forward(self,target,encoder_output,src_mask,tgt_mask,target_padding_mask):
        ...
        y=target#+positional_encodings

        assert 2<=y.dim()<=3,f'Expected the input to be 3D or 2D tensors, but got {x.dim()}D'
        if y.dim()==2:
            y=y.unsqueeze(0)
        ...
        contextualized_embeddings=self.MHA_DECODER(Q=y,K=y,V=y,mask=tgt_mask,return_attention=False).to(x.device)
        ...
        contextualized_embeddings=self.MHA_ENCODER_DECODER(Q=masked_attention_layer_output,K=x,V=x,mask=target_padding_mask,return_attention=False).to(masked_attention_layer_output.device)
